...
import { getExpenses, createExpense, deleteExpense, getCompanies } from '../services/supabase';
import { useAuth } from '../context/AuthContext';
import { Plus, Trash2, Loader2, Receipt, Search, Filter } from 'lucide-react';
...
const Expenses: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormState((prev) => ({ ...prev, description: '', amount: 0, note: '', company_id: null }));
    } catch (err) {
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 safe-area-inset">
          <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] sm:max-h-[85vh] overflow-hidden animate-fade-in flex flex-col">
            <div className="px-4 sm:px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 flex-shrink-0">
              <h3 className="font-bold text-lg text-slate-800">Новый расход</h3>
              <button
                type="button"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormState({ ...formState, expense_date: e.target.value })}
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormState({ ...formState, category: e.target.value as ExpenseCategory })}
                  >
                    {(Object.keys(CATEGORY_LABELS) as ExpenseCategory[]).map((cat) => (
                      <option key={cat} value={cat}>{CATEGORY_LABELS[cat]}</option>
                    ))}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormState({ ...formState, company_id: e.target.value || null })}
                  >
